    ...
    public function domain()
    {
        ...
        if (isset($_GET['less_thirty'])) {                //查询小于30天的
            ...
            if ($_GET['search_account_id'] == 1) {
                $a_id = $account_model->get_id_account($_GET['a_id'])['id'];
                if ($_GET['d_ip'] != '') {
                    $domain = $domain_model->search_account_id($a_id, $_GET['d_ip']);
                    $all_domain = $domain;
                } else {
                    $domain = $domain_model->get_account_domain($a_id);
                    $all_domain = $domain;
                }
                foreach ($all_account as $value) {
                    $number = $domain_model->less_thirty_day($value['id'])['less_count'];
...
//        $count = $domain_model->get_domain_count($_SESSION['user_id']);
        $this->smarty()->assign('less_count', $less_count);
        $this->smarty()->assign('all_server', $all_server);
        $this->smarty()->assign('all_domain', $all_domain);
        $this->smarty()->assign('all_account', $all_account);
        $this->smarty()->display('domain/domain.html');
    }
...
//    测试smarty
    ...
    {
        $this->smarty()->assign('qiaonima','ggggggggggggggggggggg');
