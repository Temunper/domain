...
<div>
    ...
    <table border="1" align="center" id="record-table">
        <tr>
            <td colspan="11" id="d_id"><?php $domain = $this->data['domain'];
                print_r($domain['d_name']) ?></td>
        </tr>
        <tr>
            <td></td>
            <td>二级域名</td>
        ...
        if (isset($this->data['record'])) {
            ...
            foreach ($user_record as $value) {
                ...
                $record = '<tr data-record-id="' . $value['id'] . '">' .
                    ...
                    '<td><input type="button"  value= ' . $status . ' class="record-status1"  data-record-status=' . $value['status'] . '></td>' .
                    '</tr>';
                echo $record;
            }
        } else {
...
<script>
        ...
        $(".add-line").each(function () {
            if ($(this).find(".add-record_name").val() != "") {
                var record = {
                    ...
                    "d_id":<?php $domain = $this->data['domain']; print_r($domain['id'])?>
                };
                $.ajax({
    ...
    function flash_table(record, pageSize, page_val, all_count, count) {
        ...
        var table = "<tr>" +
            "<td colspan='10' id='d_id'><?php $domain = $this->data['domain'];print_r($domain['d_name']) ?></td>" +
            "</tr>" +
            "<tr>" +
            "<td></td>" +
            "<td>二级域名</td>" +
    ...
    //    尾页
    $("body").delegate("#record-last-page", "click", function () {
        var account_name = <?php print_r($this->data['all_account'])?>;
        var page_val = $("#count").text();
        var id = <?php $domain = $this->data['domain']; print_r($domain['id']);?>;
        $.ajax({
