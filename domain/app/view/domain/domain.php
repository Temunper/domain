...
<a href="http://local.domain.cn/Domain/domain?less_thirty=1">您有
    <?php if (isset($this->data['less_count'])) {
        print_r($this->data['less_count']);
    } ?>条接近到期的域名</a>
...
<i style="right: 13px;position: relative">使用账号:<select id="search-a_id"
        ...
        foreach ($user_account as $value) {
            $ip = '<option data-acc=' . $value['id'] . '>' . $value['account_name'] . '</option>';
            echo $ip;
        }
        ?>
    </select></i>
...
<table border="1" align="center">
    ...
    <?php
    if (isset($this->data['all_domain'])) {
        $user_domain = $this->data['all_domain'];
        $user_account = $this->data['all_account'];
        foreach ($user_account as $value) {
            $ip = '<option data-acc=' . $value['id'] . '>' . $value['account_name'] . '</option>';
        ...
        foreach ($user_domain as $value) {
            ...
            $domain = '<tr class="domain-table" data-domain-id="' . $value['id'] . '">' .
                '<td><input type="checkbox" class="d_id" name="check_id"></td>' .
                '<td class="d_name1">' . $value['d_name'] . '</td>' .
                '<td class="account_name">' . $value['account_name'] . '</td>' .
                '<td>' . $value['account_type'] . '</td>' .
                '<td class="domain-status" data-domain-status="' . $value['status'] . '">' . $status . '</td>' .
                '<td><select class="d_account">'
                . '<option class="d_ip">' . $value['d_ip'] . '</option>' .
                '</select></td>' .
                '<td><select class="e-time-domain">'
                . '<option></option>' .

                '</select></td>' .
                '<td class ="d_time1">' . $value['d_time'] . '</td>' .
                '<td>' . $value['option_time'] . '</td>' .
                '<td><input type="button" value="二级域名管理" class="record-manage" domain-data=' . $value['id'] . '></td>' .
                '<td><input type="button" value="管理" class="domain-manage" domain-id=' . $value['id'] . '></td>' .
                '<td><input type="button"  value="续期" class="extend-domain"  data-domain-status="' . $value['status'] . '"></td>' .
                '</tr>';
            echo $domain;
        }
    } else {
            ...
            <div style="width: 300px;padding-left: 100px">
                ...
                <div>
                    ...
                    <i style="right: 13px;position: relative">使用账号:<select id="add-a_id"
                            ...
                            $user_account = $this->data['all_account'];
                            foreach ($user_account as $value) {
                                $ip = '<option>' . $value['account_name'] . '</option>';
                                echo $ip;
                            }
                            ?>
                        </select></i>
